...
logs ( client );
let pub = "750728566177661019";
let kayıt = "750728562771886263"
let terapi = "750728567276568698"
let sorun = "750728564889878600"
let vk = "750728568165892317"
//let dc = "729453642842636349"
//let sherlock = "701873570883829821"
//let ekip = "701478043155169370"
//let game = "729453644738592810"
let priv = "750728568509694026"
let alone = "750728570120437820"
...
client.on ( "voiceChannelJoin" , ( member , channel ) => {
    ...
    if ( ! member.roles.cache.has ( "750727279633760336" ) ) return
    const json = {
        "channel" : channel.id ,
...
client.on ( "voiceChannelLeave" , ( member , channel ) => {
    ...
    if ( data ) {
     ...
     //   else if ( channel.parentID == dc ) {
      //      db.add ( `1game:${ member.user.id }` , new Date ().getTime () - Number ( data.start ) )
      //  }
       // else if ( channel.parentID == sherlock ) {
         //   db.add ( `1game:${ member.user.id }` , new Date ().getTime () - Number ( data.start ) )
      //  }
...
client.on ( "voiceChannelSwitch" , ( member , oldChannel , newChannel ) => {
    ...
    if ( data ) {
     ...
     //   else if ( oldChannel.parentID == dc ) {
      //      db.add ( `1game:${ member.user.id }` , new Date ().getTime () - Number ( data.start ) )
      //  }
     //   else if ( oldChannel.parentID == sherlock ) {
     //       db.add ( `1game:${ member.user.id }` , new Date ().getTime () - Number ( data.start ) )
      //  }
...
client.on ( "message" , async message => {
    ...
    if ( data ) {
     ...
     //   else if ( st.channel.parentID == game ) {
      ...
      //      db.add ( `1game:${ message.author.id }` , new Date ().getTime () - Number ( data.start ) )
      //  }
       // else if ( st.channel.parentID == sherlock ) {
       //     db.add ( `1game:${ message.author.id }` , new Date ().getTime () - Number ( data.start ) )
       // }
...
client.on ( "message" , async msj => {
    ...
    const toplam = new Discord.MessageEmbed ()
        .setAuthor ( msj.guild.name , "https://media.discordapp.net/attachments/739168691089965118/740606115519397899/tenor.gif?width=398&height=224" )
        .setThumbnail ( "https://media.discordapp.net/attachments/739168691089965118/740606115519397899/tenor.gif?width=398&height=224" )
        .setColor ("030d2e")
        .setFooter ( msj.author.tag , msj.author.avatarURL ( { "dynamic" : true } ) )
        .setDescription ( `• Valerian sunucusunun ses bilgileri aşağıdadır
